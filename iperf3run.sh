...
mapfile -t THROUGHPUT_BANDWIDTH < <(seq 5 5 70)
...
function time_calc() {
  ...
  case "${1}" in
    ("all")
      ...
      eta_time_int=$(( eta_time_int_tcp + eta_time_int_udp + eta_time_int_th ))
...
function gen_csv_th() {
  ...
  jq --arg COMMENT "${COMMENT}" -r '[
    ...
    ([.end.streams[]?.sender?.mean_rtt] | add),
    ([.end.streams[]?.sender?.max_rtt] | add),
    .start.version] | @csv' TH*_TCP.json | sort -n >> results.csv
  sleep 2
  ...
  jq --arg COMMENT "${COMMENT}" -r '[
    ...
    "", "", "", "",
    .start.version] | @csv' TH*_UDP.json | sort -n >> results.csv
  sleep 2
}
...
function main() {
  ...
  case "${ARGS}" in
    ...
    (*)
      echo "Error: type emum [ all | tcp | udp | th ]"
      exit 1
      ;;
  esac
